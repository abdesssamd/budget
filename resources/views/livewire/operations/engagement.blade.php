...
class extends Component {
    ...
    // NOTE : Plus besoin de $printTemplates ici, c'est géré par le composant enfant !
    ...
    public function updatedFilterObj1($value) {
        ...
        if ($this->maxLevel >= 2) $this->filterListObj2 = $value ? BdgObj2::where('IDObj1', $value)->orderBy('Num')->get() : [];
        $this->resetPage();
    }
    public function updatedFilterObj2($value) {
        $this->filterObj3 = ''; $this->filterObj4 = ''; $this->filterObj5 = '';
        if ($this->maxLevel >= 3) $this->filterListObj3 = $value ? BdgObj3::where('IDObj2', $value)->orderBy('Num')->get() : [];
        $this->resetPage();
    }
    public function updatedFilterObj3($value) {
        $this->filterObj4 = ''; $this->filterObj5 = '';
        if ($this->maxLevel >= 4) $this->filterListObj4 = $value ? BdgObj4::where('IDObj3', $value)->orderBy('Num')->get() : [];
        $this->resetPage();
    }
    public function updatedFilterObj4($value) {
        $this->filterObj5 = '';
        if ($this->maxLevel >= 5) $this->filterListObj5 = $value ? BdgObj5::where('IDObj4', $value)->orderBy('Num')->get() : [];
        $this->resetPage();
    }
    ...
    public function openModal() {
        ...
        $this->reset('form', 'pjFiles', 'listeObj1', 'listeObj2', 'listeObj3', 'listeObj4', 'listeObj5', 'creditOuvert', 'creditDisponible');
...
<div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group">
            ...
            {{-- ICI : UTILISATION DU COMPOSANT D'IMPRESSION DYNAMIQUE --}}
            <livewire:components.print-menu 
                folder="engagement" 
                type="global" 
                btnClass="btn-secondary shadow-sm {{ $margin }}" 
                label="Liste PDF" 
            />
    ...
    <div class="card card-outline card-primary">
        <div class="card-header">
            <h3 class="card-title mb-0">{{ __('operations.engagements_history') }}</h3>
                    ...
                    <tr class="{{ $op->cf && $op->cf->VISA_cf ? 'bg-light' : '' }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                ...
                                {{-- ICI : COMPOSANT IMPRESSION INDIVIDUELLE --}}
                                <livewire:components.print-menu 
                                    folder="engagement" 
                                    :id="$op->IDOperation_Budg"
                                    type="individual" 
                                    icon="fas fa-print"
                                />
    ...
    {{-- Autres modals (Visa, PJ) inchangées --}}
