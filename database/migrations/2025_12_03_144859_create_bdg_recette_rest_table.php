...
return new class extends Migration {
    ...
    {
        Schema::create('bdg_recette_rest', function (Blueprint $table) {
            ...
            // Indexes
            $table->index('EXERCICE', 'WDIDX_bdg_recette_rest_EXERCICE');
            $table->index('IDbdg_rel_niveau', 'WDIDX_bdg_recette_rest_IDbdg_rel_niveau');
            $table->index('IDBudjet', 'WDIDX_bdg_recette_rest_IDBudjet');
            $table->index(
                ['IDObj1','IDObj2','IDObj3','IDObj4','IDObj5','EXERCICE','IDBudjet','IDSection'],
                'WDIDX_bdg_recette_rest_IDClecompose'
            );
            $table->index('IDLogin', 'WDIDX_bdg_recette_rest_IDLogin');
            $table->index('IDObj1', 'WDIDX_bdg_recette_rest_IDObj1');
            $table->index('IDObj2', 'WDIDX_bdg_recette_rest_IDObj2');
            $table->index('IDObj3', 'WDIDX_bdg_recette_rest_IDObj3');
            $table->index('IDObj4', 'WDIDX_bdg_recette_rest_IDObj4');
            $table->index('IDObj5', 'WDIDX_bdg_recette_rest_IDObj5');
            $table->index('IDSection', 'WDIDX_bdg_recette_rest_IDSection');
            $table->index('Num_operation', 'WDIDX_bdg_recette_rest_Num_operation');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('bdg_recette_rest');
    }
};
